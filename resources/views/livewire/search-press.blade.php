                            ...
                            @forelse($results as $result)
                                <div class="col-12">
                                    <div class="single-main">
                                        <a class="download mef1" wire:navigate href="{{route('pressDetail', $result->id)}}">
                                        <!-- News Head -->
                                        @if ($result->press_image != '')
                                            <div class="news-head">
                                                <img src="{{asset('storage/'.$result->press_image) }}" alt="#">
                                            </div>
                                        @endif
                                        <!-- News Title -->
                                        <h1 class="news-title mef2"><a wire:navigate href="{{route('pressDetail', $result->id)}}">{{$result->shortTitle()}}</a></h1>
                                        <!-- Meta -->
                                        <div class="meta">
                                            <div class="meta-left">
                                                <span class="date mef1">{{\Carbon\Carbon::parse($result->published_at)->locale(app()->getLocale())->translatedFormat('j F Y, g:i A');}}</span>
                                            </div>
                                            <div class="meta-right">
                                                <div class="table-bottom">
                                                    <a class="download mef1" wire:navigate href="{{route('pressDetail', $result->id)}}"><i class="fa fa-eye"></i> {{__('frontend.view')}}</a>
                                                    @if($result->press_file != '')<a class="download mef1" wire:click="download({{$result->id}})"><i class="fa fa-download"></i> {{__('frontend.doc_download')}}</a>@endif
                                                </div>
                                            </div>
                                        </div>
                                        </a>
						...
						<div class="main-sidebar">
							...
							<div class="single-widget category">
								...
								<ul class="categor-list mef2">
									...
									<li class="{{ Route::is('pressRelease') || Route::is('searchPress') ? 'active' : ''}}"><a wire:navigate href="{{ route('pressRelease')}}">{{__('frontend.press-release')}}</a></li>
